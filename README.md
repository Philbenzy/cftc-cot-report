...
#### 黄金品种专属
- **GVZ 波动率指数 vs 管理基金净持仓**：观察市场恐慌程度与投机持仓的背离关系
- **GVZ 独立走势 + GLD 周成交量**：波动率均值参考线

#### 原油品种专属
- **OVX 波动率指数 vs 非商业净持仓**：观察市场恐慌程度与投机持仓的背离关系
- **OVX 独立走势 + USO 周成交量**：波动率均值参考线
...
🎯 **特色深度数据**：黄金 GVZ 波动率、原油 OVX 波动率、铜期限结构、沪铜仓单数据
...
| 品种 | 代码 | 交易所 | 特色数据 |
|------|------|--------|----------|
| WTI原油 | wti | NYMEX | OVX 波动率 + USO 成交量 |
...
- **数据获取库**：
  ...
  - `yfinance`：GVZ/OVX 波动率指数、GLD/USO 成交量
...
- **Yahoo Finance**：GVZ/OVX 波动率指数、GLD/USO ETF 成交量
...
4. **跨市场对比**：黄金 COT + GVZ 波动率 / 原油 COT + OVX 波动率，观察恐慌与持仓背离
...
**Q2：为什么黄金品种有 GVZ 数据，原油有 OVX，其他品种没有？**
A：GVZ 和 OVX 是 CBOE 推出的波动率指数，分别针对黄金和原油市场。其他金属没有对应的波动率指数产品。

**Q3：沪铜数据是否实时？**
A：沪铜期货曲线数据每次运行脚本时获取当日最新数据，注册仓单数据为每周五上期所发布数据，不运行脚本默认每周六更新。
